...
def pinv(a, atol=None, rtol=None, return_rank=False, check_finite=True,
    ...
    pseudoinverse is a unique ``X`` that satisfies all four of these
    ...
    If ``A`` is invertable then the Moore-Penrose pseudoinverse is exactly the
    inverse of ``A`` [1]_.
    ...
    If ``A`` is not invertable then the Moore-Penrose pseudoinverse gives the
    least squares solution to ``Ax = b`` [1]_. In this example we will try to
    fit a straight line to the points ``(0,10), (1, 0), (2, 0)``.
